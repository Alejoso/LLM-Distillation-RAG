PATTERNS = {
    ...
    "to_be_transcribed": (
        r"\(Para\s+ser\s+transcrito:\s*Se\s+adjunta\s+fotocopia\s+del\s+texto"
        r"\s+íntegro\s+del\s+Instrumento\s+Internacional\s+mencionado\)\.?,?"
    ),
    "double_comma": r",,{1,}",
    # Sentence cleanup: references at start
    "ref_sentence_start": (
        r"^(?:Art\.?|Artículo|Parágrafo|Numeral|Inciso)\s+\d+[°º.]*\s*"
    ),
    ...
    "position_line": (
        r"(?:El\s+)?(?:Presidente|Secretario|Subsecretario|Ministro"
        r"|Vice\s*presidente|Procurador|Contralor|Defensor|Gobernador)\b.*?(?:\n|$)"
    ),
