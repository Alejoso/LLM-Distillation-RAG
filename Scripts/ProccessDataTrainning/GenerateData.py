...
import spacy
from CleaningPatterns import apply_patterns
...
# function to apply all cleaning patterns to the text, with special handling
# for special cases.
def clean_text(text):
...
# function to normalize text: Unicode normalization, whitespace cleanup,
# punctuation spacing.
...
# Function to process all text files in a folder, applying the full pipeline
# and collecting sentences until a limit is reached.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Generate training dataset from cleaned legal documents"
            " using spaCy sentence segmentation"
        )
    )
    parser.add_argument(
        "--input",
    ...
    if not input_dir.exists() or not input_dir.is_dir():
        print(
            f"Error: Input directory '{args.input}'"
    ...
    if args.min_chars >= args.max_chars:
        print(
            f"Error: --min-chars ({args.min_chars}) must be less than"
            f" --max-chars ({args.max_chars})"
        )
        return 1

    if args.max_sentences <= 0:
        print("Error: --max-sentences must be a positive number")
        return 1
