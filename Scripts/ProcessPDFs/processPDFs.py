...
class DocumentCleaner:
    ...
    # into a single space or newline.
    ...
    # Remove lines that are too short, except for protected ones
    # (like "I", "V", "X" which might be legal references).
    ...
    # Auxiliary function to cut text before the index section, which often
    # contains navigation and UI elements.
    ...
    # Auxiliary function to remove the SUIN-Juriscol disclaimer that appears
    # at the end of documents, which is not relevant for content and can be
    # very long.
    ...
    # Auxiliary function to protect legal structure elements like article and
    # paragraph numbering from being broken by cleaning steps. This is crucial
    # to maintain the integrity of the legal document's structure.
    def protect_legal_structure(self, text: str) -> str:
        ...
        # Fix broken paragraph with ordinal number
        # (e.g., "Parágrafo\n1º" -> "Parágrafo 1º")
        text = re.sub(
            r"(Par[áa]grafo)\s*\n\s*"
            r"(\d+[º°]?|Primero|Segundo|Tercero|Cuarto|Quinto|Sexto"
            r"|S[ée]ptimo|Octavo|Noveno|D[ée]cimo|transitorio\s+\d+[º°]?)",
            r"\g<1> \g<2>",
            text,
            flags=re.IGNORECASE,
        )

        # Fix paragraph with ordinal and dot on separate line
        # (e.g., "Parágrafo 1º\n." -> "Parágrafo 1º.")
        text = re.sub(
            r"(Par[áa]grafo\s+(?:\d+[º°]?|Primero|Segundo|Tercero|Cuarto|Quinto"
            r"|Sexto|S[ée]ptimo|Octavo|Noveno|D[ée]cimo|transitorio\s+\d+[º°]?))"
            r"\s*\n\s*\.",
            r"\g<1>.",
            text,
            flags=re.IGNORECASE,
        ...
        # Add newlines before articles, chapters, paragraphs, and "DECRETA:"
        # to ensure they are recognized as separate sections. This helps
        # maintain the logical structure of the document and improves
        # readability.
        patterns = {
            r"^\s*Art[ií]culo\s+\d+[º°]?\.?": r"\n\n\g<0>\n",
            r"^\s*Cap[ií]tulo\s+[IVXLC]+": r"\n\n\g<0>\n",
            (
                r"^\s*Par[áa]grafo(\s+(?:\d+[º°]?|Primero|Segundo|Tercero"
                r"|Cuarto|Quinto|Sexto|S[ée]ptimo|Octavo|Noveno|D[ée]cimo"
                r"|transitorio\s+\d+[º°]?))?\.?"
            ): r"\n\g<0>\n",
            r"^\s*DECRETA:": r"\n\nDECRETA:\n",
        }
    ...
    # Main function to clean text by applying all cleaning steps in a logical
    # order. This function ensures that the document is cleaned while
    # preserving the essential legal structure and content, making it suitable
    # for use in RAG systems.
    ...
    def cleaning_report(self, original: str, cleaned: str) -> dict:
        return {
            ...
            "reduction_ratio": round(1 - len(cleaned) / max(len(original), 1), 4),
        }
    ...
    def extract_text_from_pdf(self, pdf_path: str) -> str:
        ...
        if not PDF_SUPPORT:
            raise ImportError(
                "pypdfium2 is not installed. Install it with: pip install pypdfium2"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Clean documents for RAG systems")
    ...
    if input_path.is_file():
        ...
        # Process directory
        processed = cleaner.process_directory(args.input, args.output, args.extensions)
