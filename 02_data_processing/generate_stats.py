...
# Reads .txt files from the given directories and extracts the quality score
# and status from each
...
def plot_extracted_files(sitemap_total: int, html_count: int, output_path: Path):
    ...
    ax_donut.text(
        0, -0.16, "exitoso", ha="center", va="center", fontsize=10, color="#999999"
    )
    ...
    fig.suptitle(
        "Archivos extraidos por Web Scraping", fontsize=16, fontweight="bold", y=0.97
    )
...
# Generates a bar chart showing the distribution of files by quality level
# (HIGH, MEDIUM, LOW, DEFECTIVE)
def plot_quality_distribution(records: list[dict], output_path: Path):
    ...
    for bar, val in zip(bars, values, strict=False):
        ...
        ax.text(
            bar.get_x() + bar.get_width() / 2,
            bar.get_height() + max(values) * 0.065,
            f"({pct:.1f}%)",
            ha="center",
            fontsize=9,
            color="#999999",
    ...
    fig.suptitle(
        "Distribucion de archivos por calidad de limpieza",
        fontsize=16,
        fontweight="bold",
        y=0.97,
    )
...
def plot_usable_vs_unusable(usable: int, unusable: int, output_path: Path):
    ...
    for bar, val, pct in zip(bars, values, [usable_pct, unusable_pct], strict=False):
        ax_bar.text(
            bar.get_x() + bar.get_width() / 2,
            bar.get_height() + max(values) * 0.02,
            f"{val:,}",
    ...
    ax_donut.text(
        0, -0.18, "usables", ha="center", va="center", fontsize=10, color="#999999"
    )
    ...
    fig.suptitle(
        "Archivos usables vs no usables", fontsize=16, fontweight="bold", y=0.97
    )
    fig.tight_layout(rect=[0, 0, 1, 0.92])
    fig.savefig(output_path, dpi=180, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    plot_extracted_files(
        sitemap_total, html_count, output_dir / "01_archivos_extraidos.png"
    )
    plot_quality_distribution(records, output_dir / "02_distribucion_calidad.png")
    plot_usable_vs_unusable(
        usable_count, unusable_count, output_dir / "03_usables_vs_no_usables.png"
    )
